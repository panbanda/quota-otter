...
function updateTray(){if(!native)return;const list=grouped();const oa=summarize(list,'openai'),cl=summarize(list,'claude');const group=state.groups.find(g=>g.id===selected)?.name||'All accounts';const title=`${demo?'DEMO · ':''}OA ${oa.ready}/${oa.total} · CL ${cl.ready}/${cl.total}`;const summary=`${demo?'DEMO: ':''}${group} | OpenAI: ${oa.ready}/${oa.total} available; Claude: ${cl.ready}/${cl.total} reported available | ${oa.resets??'?'} reset credits (${oa.resetKnown}/${oa.total} reporting)`;invoke('update_tray',{title,summary}).catch(()=>{});}
